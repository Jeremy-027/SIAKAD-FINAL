...
mahasiswa::mahasiswa(string id, string nama, int dd, int mm, int yy, string nrp, string DPT, int tahunmasuk, vector<string> matkul)
		: person(id, nama, dd, mm, yy), nrp(nrp), DPT(DPT), tahunmasuk(tahunmasuk), matkul(matkul)
{
	this->ipk = 0.0;
	this->semesterke = 1;
	this->skslulus = 0;
...
void mahasiswa::setMatkul(string matkul)
{
	this->matkul.push_back(matkul);
}

vector<string> mahasiswa::getMatkul()
{
	return this->matkul;
}
